class DIYTool {
    ...
    updateRGBLUT() {
        ...
        for (let i = 0; i < 1024; i++) {
            ...
            //Math.round会取整，精度会丢失，但是已经有1024字节LUT了，所以精度相差不大
            this.rgbLUT.red[i] = Math.round(Math.pow(originalValue / 255.0, gammaRMapped) * 255.0);
            this.rgbLUT.green[i] = Math.round(Math.pow(originalValue / 255.0, gammaGMapped) * 255.0);
            this.rgbLUT.blue[i] = Math.round(Math.pow(originalValue / 255.0, gammaBMapped) * 255.0);
        }
    }
    ...
    handleFltImport(event) {
        ...
        reader.onload = (e) => {
            ...
            // 更新LUT表
            this.rgbLUT.red = fltParams.redLUT;
            this.rgbLUT.green = fltParams.greenLUT;
            this.rgbLUT.blue = fltParams.blueLUT;
    ...
    parseFltFile(content) {
        ...
        // 解析LUT表数据
        fltParams.redLUT = lines[currentLine++].trim().split(' ').map(Number);
        fltParams.greenLUT = lines[currentLine++].trim().split(' ').map(Number);
        fltParams.blueLUT = lines[currentLine++].trim().split(' ').map(Number);
    ...
    async handleExport() {
        try {
            ...
            // 添加LUT表数据
            content += this.rgbLUT.red.join(' ') + '\n'; // R通道数据
            content += this.rgbLUT.green.join(' ') + '\n'; // G通道数据
            content += this.rgbLUT.blue.join(' ') + '\n'; // B通道数据
